...
---
# Practica 2. DESCRIPCIÓN DE R: PRIMEROS PASOS
## Funciones de ayuda
```{r}
 help(mean)
 ?mean
...
```{r}
doub<-4#Escribimos doub <- en la consola y pulsamos la tecla enter
doub=4#asigna el valor 4 ala variable doub
doub #muestra el valor de la variable doub
int = as.integer(4)
int #muestra el valor de la variable int
logi = FALSE
logi
char1="hola" #cuidado con las comillas son las de las teclas del numero 2
char1
char2<-"5 + 6"
char2
typeof(doub)
...
9%/%6
9%%6
a= 2+2 #resultado de 2+2 se asigna ala variable a
b= 3*2 #resultado de 3*2 se asigna ala variable b
b-a ->c#resultado de b-a se asigna ala variable c
a
b
c
a==5
...
split(peso, trat)
split(peso, trat)$A
x <- 1:15; length(x) # varias instrucciones en un mismo renglón separadas
...
substr("el", "sol", "brilla", 2, 4)
x <- paste(LETTERS[1:7], collapse = "")
substr(x, 3, 7) <- c("el", "sol", "brilla")
...
factor2 <- factor(c("alto", "bajo", "medio"), levels = c("alto", "bajo", "medio")) #cambia el orden de las etiquetas
